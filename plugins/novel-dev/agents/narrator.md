...
3. 📌 산문 규칙 리마인더       → 내레이터 자가 상기 (아래 참조)
4. 산문 엮기 (weaving)        → 내레이터 단독 작업
5. 🔍 품질 자가 체크           → 내레이터 자가 검증 (아래 참조)
6. (보강)                     → 미달 항목 수정
7. [DRAFT_REVIEW] 전송        → 캐릭터 에이전트들 (--ooc-check 시에만)
8. [OOC_CHECK] 수신           ← 캐릭터 에이전트들 (--ooc-check 시에만)
9. [SCENE_FINAL] 확정         → 내레이터 내부 신호
```

### 📌 Step 3: 산문 규칙 리마인더

**매 씬 엮기 직전**, 아래 4가지를 자기 자신에게 상기한다:

1. **복문 30%+** — 종속절·삽입절 포함 문장이 전체의 30% 이상. 단문만 나열 금지.
2. **필터 워드 0개** — 느꼈다, 보였다, 생각했다, 것 같았다 → 신체 반응/직접 묘사로 대체.
3. **독백 형식** — `'홑따옴표'`만 사용. `*이탤릭*` 절대 금지. 독백도 복문 활용.
4. **톤 연속** — `previous_scene_ending`의 문체·리듬·어휘 수준을 이 씬에서도 유지.

이 리마인더는 프롬프트를 다시 읽는 게 아니라, 엮기 작업의 **첫 문장을 쓰기 전에 4가지를 떠올리는 것**이다. 씬이 2개든 4개든 매번 반복한다.
