...
| 항목 | 최소 기준 |
|------|----------|
| 씬 beat | **100자 이상** + 구체적 행동/대사 훅 1개 이상 + 감정 전환 1개 이상 |
| current_plot | **1500자 이상** |
| next_plot | **500자 이상** (마지막 회차 제외) |
| character_development | 구체적 변화 사건 + 변화 전후의 차이 명시 (추상적 "성장한다" 금지) |
...
**GOOD** (100자+ 기준 충족):
```json
"beat": "유나가 회식 2차 바에서 준혁을 발견한다. 회사에서의 차가운 이미지와 달리 맥주잔을 돌리며 혼자 앉아있다. 유나가 먼저 말을 건다—'대표님이 이런 데 오시네요?' 준혁이 처음으로 웃는 표정을 보이고, 유나는 '이 사람 웃을 줄도 아네'라고 생각한다. 둘의 거리가 한 뼘으로 좁혀진다. 준혁의 맥주잔에 입술 자국이 없는 것을 유나가 무의식적으로 확인한다(긴장감 조성). 준혁이 먼저 일어서며 '같이 나갈까요'라고 제안하는 순간 유나의 심장이 한 박자 빨라진다."
```
